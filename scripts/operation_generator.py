...
def add_glo():
    ...
    # header ###################################################################
    asm.append("#define add_glo(c_glo, a_glo, b_glo, tid) {");
    asm.append("asm(");
    ...
    asm.append(":");
    for i in range(max_bignum_number_of_words - 1):
        asm.append("\"=r\"(c_glo[" + "COAL_IDX(" + str(i) + ", tid)]),");
    asm.append("\"=r\"(c_glo[" + "COAL_IDX(" + str(max_bignum_number_of_words - 1) + ", tid)]),");

    asm.append(":");
    for i in range(max_bignum_number_of_words):
        asm.append("\"r\"(a_glo[" + "COAL_IDX(" + str(i) + ", tid)]),");

    for i in range(max_bignum_number_of_words - 1):
        asm.append("\"r\"(b_glo[" + "COAL_IDX(" + str(i) + ", tid)]),");
    asm.append("\"r\"(b_glo[" + "COAL_IDX(" + str(max_bignum_number_of_words - 1) + ", tid)])");
...
def add_glo_2():
    asm = add_loc();
    asm = [line.replace('add_loc', 'add_glo') for line in asm];
    asm = [line.replace('c_loc', 'c_glo') for line in asm];
    asm = [line.replace('a_loc', 'a_glo') for line in asm];
    asm = [line.replace('b_loc', 'b_glo') for line in asm];
    asm = [line.replace('c_loc', 'c_glo') for line in asm];
    asm = [re.sub("_loc[\d+]", "", line) for line in asm];
    return asm;
