...
We found that having the operands in 2 different memory locations was better.
We initially though the interleaved technique was going to be better, as the GPU could
read both operands with one global memory read, however, when compiling to PTX, we
see that there are actually 2 separate load instructions performed.
...
algorithm defined in the Handbook of Applied Cryptography.
From what we understood, we were able to implement the shifted additions before the
right-shift operation.
We belive that the right-shift is only possible to perform if the least significant bits of
the bignum are all zero, otherwise we would lose information from the shift.
Until the right shift, we do obtain some value with only 0s in it (on the least
significant bits).
We even checked that the shifting operation is performing its work correctly, and
it was the case.

However, our operation tester kept telling us that the operation was false.
From there, we have 2 options:
\begin{itemize}
\item Our implementation of the Montgomery reduction is wrong, and is not computing
...
\item The second option is that we have misunderstood how Montgomery reductions
actually work, and are therefore not providing operands which satisfy its conditions.
\end{itemize}

We belive that we are in the case of the second option.
