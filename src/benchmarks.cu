...
__global__ void montgomery_reduction_loc_kernel(uint32_t* dev_c_mon, uint32_t* dev_a_mon, uint32_t* dev_b_mon, uint32_t* dev_m);
...
void sub_m_benchmark()
{
    ...
    write_coalesced_bignums_to_file(SUB_M_RESULTS_FILE_NAME, host_c, false);
...
__global__ void montgomery_reduction_loc_kernel(uint32_t* dev_c_mon, uint32_t* dev_a_mon, uint32_t* dev_b_mon, uint32_t* dev_m, uint32_t* dev_inv_R)
{
    uint32_t tid = blockIdx.x * blockDim.x + threadIdx.x;

    uint32_t a_mon[MIN_BIGNUM_NUMBER_OF_WORDS];
    uint32_t b_mon[MIN_BIGNUM_NUMBER_OF_WORDS];
    uint32_t c_mon[MIN_BIGNUM_NUMBER_OF_WORDS];
    uint32_t m[MIN_BIGNUM_NUMBER_OF_WORDS];
    uint32_t inv_R[MIN_BIGNUM_NUMBER_OF_WORDS];

    for (uint32_t i = 0; i < MIN_BIGNUM_NUMBER_OF_WORDS; i++)
    {
        a_mon[i] = dev_a_mon[COAL_IDX(i, tid)];
        b_mon[i] = dev_b_mon[COAL_IDX(i, tid)];
        m[i] = dev_m[COAL_IDX(i, tid)];
    ...
    for (uint32_t i = 0; i < MIN_BIGNUM_NUMBER_OF_WORDS; i++)
    {
        dev_c_mon[COAL_IDX(i, tid)] = c_mon[i];
    }
}
...
void montgomery_reduction_benchmark()
{
    ...
    read_coalesced_bignums_from_file(COALESCED_A_MON_FILE_NAME, host_a, false);
    read_coalesced_bignums_from_file(COALESCED_B_MON_FILE_NAME, host_b, false);
    read_coalesced_bignums_from_file(COALESCED_M_FILE_NAME, host_m, false);
    read_coalesced_bignums_from_file(INVERSE_R_FILE_NAME, host_inv_R, false);
    ...
    printf("Benchmarking \"montgomery reduction\" on GPU ... ");
    fflush(stdout);

    // execute kernel
    montgomery_reduction_loc_kernel<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK>>>(dev_c, dev_a, dev_b, dev_m, dev_inv_R);
    ...
    write_coalesced_bignums_to_file(MONTGOMERY_REDUCTION_RESULTS_FILE_NAME, host_c, false);
