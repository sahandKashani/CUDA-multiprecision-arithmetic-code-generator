 ...
 * @param  str string to be tested (must be non-NULL).
 ...
 * @param  array NULL-terminated array to be tested (must be non-NULL).
...
/**
 * Converts a uint32_t to a binary string. The returned string has a length of
 * BITS_PER_WORD.
 ...
 * length BITS_PER_WORD. The returned array will have MAX_BIGNUM_NUMBER_OF_WORDS
 * elements, with the least significant bits of the bignum at index 0, and the
 * most significant bits at index (MAX_BIGNUM_NUMBER_OF_WORDS - 1).
 * @param  bignum bignum to convert (must be non-NULL).
...
char** bignum_to_binary_string_array(uint32_t* bignum)
{
    ...
    // one of the MAX_BIGNUM_NUMBER_OF_WORDS elements of the bignum.
    char** words = (char**) calloc(MAX_BIGNUM_NUMBER_OF_WORDS + 1, sizeof(char*));
    assert(words != NULL);

    words[MAX_BIGNUM_NUMBER_OF_WORDS] = NULL;
    ...
    assert(words != NULL);
    assert(number_of_elements_in_binary_string_array(words) == MAX_BIGNUM_NUMBER_OF_WORDS);
    for (uint32_t i = 0; i < MAX_BIGNUM_NUMBER_OF_WORDS; i++)
    {
        assert(words[i] != NULL);
...
/**
 * Concatenates each of the binary strings in a NULL-terminated string to form a
 * single binary string of length TOTAL_BIT_LENGTH. The input array must contain
 * MAX_BIGNUM_NUMBER_OF_WORDS binary strings, each of which is BITS_PER_WORD
 * characters long.
 * @param  words binary string array to concatenate together (must be non-NULL).
 * @return       concatenated binary string (guaranteed to be non-NULL).
 */
char* concatenate_binary_string_array(char** words)
{
    assert(words != NULL);
    ...
    for (uint32_t i = 0; i < MAX_BIGNUM_NUMBER_OF_WORDS; i++)
    {
        src = words[MAX_BIGNUM_NUMBER_OF_WORDS - i - 1];
 ...
 * @param  bignum bignum to convert (must be non-NULL).
 * @return        binary string representation of the bignum (guaranteed to be
 *                non-NULL).
 */
 ...
 * function reallocates the memory of the string to have enough space for
...
void flip_string_array(char** array)
{
    ...
    {
        tmp = array[i];
        array[i] = array[MAX_BIGNUM_NUMBER_OF_WORDS - 1 - i];
        array[MAX_BIGNUM_NUMBER_OF_WORDS - 1 - i] = tmp;
    }
}

/**
 * Converts a binary string of length TOTAL_BIT_LENGTH to a NULL-terminated
 * binary string array with MAX_BIGNUM_NUMBER_OF_WORDS elements, each of length
 * BITS_PER_WORD.
...
char** binary_string_to_binary_string_array(char* str)
{
    ...
    // cut str into MAX_BIGNUM_NUMBER_OF_WORDS pieces, each of which is
    // BITS_PER_WORD long

    // array of BITS_PER_WORD length strings
    char** str_words = (char**) calloc(MAX_BIGNUM_NUMBER_OF_WORDS + 1, sizeof(char*));
    assert(str_words != NULL);

    str_words[MAX_BIGNUM_NUMBER_OF_WORDS] = NULL;

    // allocate each one of the strings and fill them up
    for (uint32_t i = 0; i < MAX_BIGNUM_NUMBER_OF_WORDS; i++)
    {
        str_words[i] = (char*) calloc(BITS_PER_WORD + 1, sizeof(char));
        assert(str_words[i] != NULL);
    ...
    assert(number_of_elements_in_binary_string_array(str_words) == MAX_BIGNUM_NUMBER_OF_WORDS);
    for (uint32_t i = 0; i < MAX_BIGNUM_NUMBER_OF_WORDS; i++)
    {
 ...
 * MAX_BIGNUM_NUMBER_OF_WORDS elements, each of which is BITS_PER_WORD characters
 * long. This type of array is returned by a call to
...
{
    assert(in != NULL);
    transpose(in, MAX_BIGNUM_NUMBER_OF_WORDS, NUMBER_OF_BIGNUMS);
}
...
{
    assert(in != NULL);
    transpose(in, NUMBER_OF_BIGNUMS, MAX_BIGNUM_NUMBER_OF_WORDS);
}
...
/**
 * Converts a positive mpz_t to a binary string of length TOTAL_BIT_LENGTH. The
 * number is padded with 0s until it is TOTAL_BIT_LENGTH long.
 * @param  number number to be converted.
